"use strict";
 ...
 var ga4blogger_track_postview = true;
...
var ga4blogger = {
	...
	track_postview : function (){
		window.ga4blogger_track_postview = (window.ga4blogger_track_postview && ga4blogger.path === "/"); 
		return window.ga4blogger_track_postview;
	},
	...
	apply_selector : function () {
		jQuery(document).ready(function () {
			try {
				...
				jQuery.extend(
					jQuery.expr[':'], {
						hrefregex: function (obj, i, m) {
							if(!obj.href) { 
								return false;
							}
							var r = new RegExp(m[3], 'i');
							return r.test(obj.href);
						},
				...
				if (ga4blogger.track_postview()) {
					var url, 
						permalinks = [], 
						url_prefix = (ga4blogger.protocol + '//' + ga4blogger.host + '/');
					jQuery("a:not(:external):not([href*='#']):hrefregex('\\/\\d{4}\\/\\d{2}\\/')").each(function(){
